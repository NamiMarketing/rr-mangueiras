/**
 * Adiciona os 45 produtos da categoria "Linha Pneumática" (Figma, node 909:524),
 * divididos em 11 subcategorias — quatro delas agrupadas sob uma subcategoria
 * pai ("Conexões de latão" e "Conexões plásticas").
 *
 * A categoria fica em layout "auto": cada grupo decide sozinho como aparecer.
 * Os tubos têm descrição e saem no layout padrão (nome, imagem e descrição);
 * as conexões, sem descrição, saem no compacto em duas colunas.
 *
 * As 9 imagens de tubo vêm de scripts/linha-pneumatica-src/merged/, onde as
 * bolinhas de cores disponíveis já estão embutidas na foto — geradas por
 * scripts/merge-bolinhas.mjs. As demais vêm da raiz de linha-pneumatica-src/.
 *
 * Os dados (nomes, descrições, subcategorias e ordem) foram extraídos do Figma
 * e ficam em linha-pneumatica-dados.json, ao lado deste arquivo.
 *
 * NÃO destrutivo: reaproveita a categoria (que já existe, vazia) e pula
 * produtos cujo slug já exista (idempotente).
 *
 * Uso:
 *   node scripts/seed-linha-pneumatica.mjs         # dry-run
 *   node scripts/seed-linha-pneumatica.mjs --yes   # aplica
 *
 * Requer no .env:
...
const NOME_CATEGORIA = "Linha Pneumática";
const ORDEM_CATEGORIA = 3;
const SRC = "linha-pneumatica-src";

const { subcategorias: SUBCATEGORIAS, produtos: PRODUTOS } = JSON.parse(
  readFileSync(resolve(__dirname, "linha-pneumatica-dados.json"), "utf8")
);
...
function caminhoImagem(item) {
  return resolve(__dirname, SRC, item.merged ? "merged" : ".", item.arquivo);
}

async function uploadImage(item) {
  const buffer = readFileSync(caminhoImagem(item));
...
async function main() {
  ...
  for (const item of PRODUTOS) readFileSync(caminhoImagem(item));
  const merged = PRODUTOS.filter((p) => p.merged).length;
  console.log(`🖼️  ${PRODUTOS.length} imagens encontradas (${merged} com bolinhas embutidas)\n`);
  ...
  for (let i = 0; i < PRODUTOS.length; i++) {
    ...
    const slug = slugify(`${item.nome}-linha-pneumatica`);
    ...
    const imagem = await uploadImage(item);
    await client.create({
      _type: "produto",
      nome: item.nome,
